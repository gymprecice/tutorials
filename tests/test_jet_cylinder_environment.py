...
def patch_env_helpers(mocker):
    mocker.patch(
        "closed_loop_AFC.jet_cylinder.environment.get_interface_patches",
        return_value=[],
    )
    mocker.patch(
        "closed_loop_AFC.jet_cylinder.environment.get_patch_geometry",
        return_value={},
    )
...
ENVIRONMENT_CONFIG = {
    ...
    "controller": {
        "read_from": {},
        "write_to": {"jet1": "Velocity", "jet2": "Velocity"},
    },
}
...
class TestJetCylinder2D:
    ...
    def make_env(self):
        from closed_loop_AFC.jet_cylinder.environment import JetCylinder2DEnv
    ...
    def test_get_action(self, testdir, mock_adapter, mocker):
        ...
        sim_engine = path.join(
            TestJetCylinder2D.gymprecice_tutorials_dir,
            "closed_loop_AFC/jet_cylinder/physics-simulation-engine",
        )
        ...
        check = {
            "jet1_Velocity": np.array_equal(
                output0["jet1"], np.negative(output1["jet1"])
            ),
            "jet2_Velocity": np.array_equal(
                output0["jet2"], np.negative(output1["jet2"])
            ),
        }
        assert all(check.values())
